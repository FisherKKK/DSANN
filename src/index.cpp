...
/** 整体的操作思路:
 *      1. 将config中的部分配置值进行赋值
 *      2. 接下来进行config的冲突检查, 这里相当于是二次检查了
 *      3. 对部分配置值进行更新, 以及保存data_store, graph_store和pq_data_store
 *      4. 构建了_lock[MAX_POINT], 也就是说现在每个点都有了一个锁
 *TODO  5. 根据search_param构建query的配置信息
*/
      ...
      _delete_set(new tsl::robin_set<uint32_t>), _conc_consolidate(index_config.concurrent_consolidate)
{
    ...
    if (index_config.index_write_params != nullptr)
    {
        ...
        //! 索引搜索的配置信息, 不为空才进行配置, 但应该是比较重要的部分
                         ...
                         defaults::GRAPH_SLACK_FACTOR * 1.05)))
{
    ...
    {   // 如果没有启用pq_dist, 那么pq_data_store和data_store将会一致
...
/**
 * 这里相当于田间了num_threads个InMemQueryScratch对象, 并将它们压入队列中
 */
template <typename T, typename TagT, typename LabelT>
...
template <typename T, typename TagT, typename LabelT> uint32_t Index<T, TagT, LabelT>::calculate_entry_point(size_t num)
{
    // REFACTOR TODO: This function does not support multi-threaded calculation of medoid.
    // Must revisit if perf is a concern.
    return _data_store->calculate_medoid_num(num);
}
...
template <typename T, typename TagT, typename LabelT>
std::vector<std::pair<float, uint32_t>> Index<T, TagT, LabelT>::path_to_fixed_point_early_stop_disk(int k,
                                                                                               ...
                                                                                               std::vector<uint32_t >& agg2id,
                                                                                               std::vector<std::vector<uint32_t>> &ivf,
                                                                                               std::vector<uint32_t> &ivf_sz,
                                                                                               float radius, float scale_factor,
                                                                                               int fd,
                                                                                               std::unique_ptr<AsyncIOManager<T>> &io_mgr) {
    using Element = std::pair<float, uint32_t>;
    std::vector<Element> path;
    std::vector<Element> result(k);
    // preserve 10 * Lsize
    path.reserve(10 * Lsize);
    // 扩展的节点
    std::vector<Neighbor> &expanded_nodes = scratch->pool();
    // 最优节点的优先队列
    NeighborPriorityQueue &best_L_nodes = scratch->best_l_nodes();
    // 事先准备Lsize个
    best_L_nodes.reserve(Lsize); // Make termination condition is radius judgement
    // 已经访问过的节点
    ...
    const int keep_minimal_limit = 8; // the keep times which considered as the minimal
    int keep_minimal_times = 0; // current keep times
    uint32_t maybe_minimal_id = -1; // the kept id
    float maybe_minimal_distance = std::numeric_limits<float>::max(); // the kept distance

    bool is_hit = false; // is the minimal hit
    bool initial_load = false;

    // 如果最优节点中存在还没有扩展的节点(可以理解是还没有找邻居的节点)
    while (best_L_nodes.has_unexpanded_node())
    {
        ...
        if (is_hit) {
            if (!initial_load) {
                for (auto &[d, id]: path) {
                    if (ivf_sz[id] > 0 && nbr.distance <= scale_factor * (maybe_minimal_distance + 2 * radius))
                        io_mgr->read(fd, id);
                }
                initial_load = true;
            } else if (ivf_sz[n] > 0) {
            ...
            if (nbr.distance > scale_factor * (maybe_minimal_distance + 2 * radius)) {
                best_L_nodes.clear();
                break;
            }
        }
        else if (keep_minimal_times >= keep_minimal_limit) {
            is_hit = true;
        }
        else if (nbr.distance < maybe_minimal_distance) {
            maybe_minimal_distance = nbr.distance;
            maybe_minimal_id = n;
            keep_minimal_times = 0;
        }
        else {
            keep_minimal_times++;
        }
        ...
        path.emplace_back(nbr.distance, nbr.id);
        ...
        else
        {
            // 获取当前准备扩展节点的锁
            _locks[n].lock();
            // 到目前为止 _graph_store中还是空的啊, 返回的是第n个节点对应的vector, 也就是邻居列表
            auto nbrs = _graph_store->get_neighbours(n);
            //? 为啥解锁啊
            _locks[n].unlock();
            // 后面可能这个节点会有邻居, 但是刚开始的时候什么都没有
            for (auto id : nbrs)
            {
                assert(id < _max_points + _num_frozen_pts);
    ...
    int trans_sz = k < path.size() ? k : path.size();
    std::nth_element(path.begin(), path.begin() + trans_sz, path.end());
    std::transform(path.begin(), path.begin() + trans_sz, result.begin(), [&](Element &e) -> Element {
        return {e.first, agg2id[e.second]};
    });
    std::make_heap(result.begin(), result.end(), std::less<Element>());
    io_mgr->process(k, result);
...
//TODO: !maybe bugs
template <typename T, typename TagT, typename LabelT>
std::vector<std::pair<float, uint32_t>> Index<T, TagT, LabelT>::path_to_fixed_point_early_stop(InMemQueryScratch<T> *scratch, const uint32_t Lsize,
                                                                                               const std::vector<uint32_t> &init_ids, bool use_filter,
                                                                                               const std::vector<LabelT> &filter_labels, bool search_invocation,
                                                                                               std::vector<float> &radius, float scale_factor) {
    using Element = std::pair<float, uint32_t>;
    std::vector<Element> path;
    // preserve 10 * Lsize
    path.reserve(10 * Lsize);
    ...
    while (best_L_nodes.has_unexpanded_node())
    {
        ...
        if (is_hit) {
            if (nbr.distance > scale_factor * (maybe_minimal_distance + 2 * radius[maybe_minimal_id] + radius[n]))
                break;
        }
        else if (keep_minimal_times >= keep_minimal_limit) is_hit = true;
        else if (nbr.distance - radius[n] < maybe_minimal_distance) {
            maybe_minimal_distance = nbr.distance - radius[n];
            maybe_minimal_id = n;
            keep_minimal_times = 0;
        }
        else keep_minimal_times++;
...
std::vector<std::pair<float, uint32_t>> Index<T, TagT, LabelT>::path_to_fixed_point_early_stop(InMemQueryScratch<T> *scratch, const uint32_t Lsize,
    ...
    NeighborPriorityQueue &best_L_nodes = scratch->best_l_nodes();
    // 事先准备Lsize个
    best_L_nodes.reserve(Lsize * 2); // Make termination condition is radius judgement
    // 已经访问过的节点
    tsl::robin_set<uint32_t> &inserted_into_pool_rs = scratch->inserted_into_pool_rs();
    boost::dynamic_bitset<> &inserted_into_pool_bs = scratch->inserted_into_pool_bs();
    std::vector<uint32_t> &id_scratch = scratch->id_scratch();
    ...
    const int keep_minimal_limit = 4; // the keep times which considered as the minimal
    int keep_minimal_times = 0; // current keep times
    uint32_t maybe_minimal_id = -1; // the kept id
    float maybe_minimal_distance = std::numeric_limits<float>::max(); // the kept distance
...
template <typename T, typename TagT, typename LabelT>
std::vector<std::pair<float, uint32_t>> Index<T, TagT, LabelT>::path_to_fixed_point(InMemQueryScratch<T> *scratch, const uint32_t Lsize, const std::vector<uint32_t> &init_ids, bool use_filter,
                                                                                    const std::vector<LabelT> &filter_labels, bool search_invocation) {
    using Element = std::pair<float, uint32_t>;
    std::vector<Element> path;
    // preserve 10 * Lsize
...
{
    // 这里init_ids = [_start + frozen(0)]
    const std::vector<uint32_t> init_ids = get_init_ids();
    const std::vector<LabelT> unused_filter_label;

    if (!use_filter)
    {
        // 获取对应位置的向量, 并copy到scratch中, 也就是说scratch中包含了对齐后的当前向量
        _data_store->get_vector(location, scratch->aligned_query());
        // 这里相当于操作了, 是由又, 没有任何操作, best_search, expanded中多了个初始点
...
template <typename T, typename TagT, typename LabelT> void Index<T, TagT, LabelT>::link()
{
    ...
    //! 相当于有frozen点就用这个点作为入口, 否则计算入口点
    if (_num_frozen_pts > 0)
        _start = (uint32_t)_max_points;
    else
        // 这里就是计算入口点
        _start = calculate_entry_point(_nd);

    diskann::Timer link_timer;

    // 将如下的块分为2048个, 然后每个线程并发进行执行
#pragma omp parallel for schedule(dynamic, 2048)
    for (int64_t node_ctr = 0; node_ctr < (int64_t)(visit_order.size()); node_ctr++)
    {
        ...
        // manager相当于为多线程分配query_scratch, 每个线程都会取走一个, 如果没有了就阻塞, 直到notify
        ScratchStoreManager<InMemQueryScratch<T>> manager(_query_scratch);
        // 返回了一个scratch
...

        {
            ...
            // 这里相当于将prune_list中的节点作为邻居, 但是现在就只有一个初始节点啊, 什么玩意
...
void Index<T, TagT, LabelT>::build(const char *filename, const size_t num_points_to_load, const std::vector<TagT> &tags)
{
    ...
    // 载入数据, 本质上就是从offset处读取数据: 对齐 + 数据预处理(针对cosine距离)
...
/** 索引的构建:
 *      1. 调用build(file, data_num, default参数)
 *      2.
*/
template <typename T, typename TagT, typename LabelT>
...
DISKANN_DLLEXPORT std::pair<uint32_t, uint32_t> Index<T, TagT, LabelT>::search_along_path_early_stop_disk(const T *query,
                                                                                                          size_t K,
                                                                                                          size_t L,
                                                                                                          std::vector<uint32_t>& agg2id,
                                                                                                          std::vector<std::vector<uint32_t>> &ivf,
...
//    }
    ...
    auto result = path_to_fixed_point_early_stop_disk(K, scratch, L, init_ids, false,
                                               unused_filter_label, true, 
                                               agg2id, ivf, ivf_sz, radius, 
                                               early_stop_factor, fd, io_mgr);
...
//        result[i].first = path[i].first;
//        result[i].second = (IdType) path[i].second;
//    }
    for (size_t i = 0; i < K; i++) {
        indices[i] = (IdType)result[i].second;
        if (distances != nullptr)
            distances[i] = result[i].first;
    }
    return {};
}
...
template<typename T, typename TagT, typename LabelT>
template<typename IdType>
std::pair<uint32_t, uint32_t> Index<T, TagT, LabelT>::search_with_path_early_stop(const T *query,
                                                                                  std::vector<std::pair<float, IdType>> &result ,
                                                                                  ...
                                                                                  float scale_factor)
{
    ...
    //    auto path = path_to_fixed_point(scratch, L, init_ids, false, unused_filter_label, true);

    auto path = path_to_fixed_point_early_stop(scratch, L, init_ids, false,
                                               unused_filter_label, true, radius, scale_factor);
...
template<typename T, typename TagT, typename LabelT>
template<typename IdType>
std::pair<uint32_t, uint32_t> Index<T, TagT, LabelT>::search_with_path(const T *query, std::vector<std::pair<float, IdType>> &result, const size_t K, const uint32_t L)
{
    if (K > (uint64_t)L)
    {
        throw ANNException("Set L to a value of at least K", -1, __FUNCSIG__, __FILE__, __LINE__);
...
template <typename T, typename TagT, typename LabelT>
void Index<T, TagT, LabelT>::sample_neighbor_distance(location_t id, std::vector<float>& dists)
{
    const auto& neighbors =  _graph_store->get_neighbours(id);
    for (auto nn: neighbors)
    {
        dists.push_back(_data_store->get_distance(nn, id));
    }
}
...
template DISKANN_DLLEXPORT std::pair<uint32_t, uint32_t> Index<float, uint64_t, uint32_t>::search_with_path<uint64_t>(
    const float *query, std::vector<std::pair<float, uint64_t>>& result, const size_t K, const uint32_t L);
template DISKANN_DLLEXPORT std::pair<uint32_t, uint32_t> Index<float, uint64_t, uint32_t>::search_with_path<uint32_t>(
    const float *query, std::vector<std::pair<float, uint32_t>>& result, const size_t K, const uint32_t L);
template DISKANN_DLLEXPORT std::pair<uint32_t, uint32_t> Index<uint8_t, uint64_t, uint32_t>::search_with_path<uint64_t>(
    const uint8_t *query, std::vector<std::pair<float, uint64_t>>& result, const size_t K, const uint32_t L);
template DISKANN_DLLEXPORT std::pair<uint32_t, uint32_t> Index<uint8_t, uint64_t, uint32_t>::search_with_path<uint32_t>(
    const uint8_t *query, std::vector<std::pair<float, uint32_t>>& result, const size_t K, const uint32_t L);
template DISKANN_DLLEXPORT std::pair<uint32_t, uint32_t> Index<int8_t, uint64_t, uint32_t>::search_with_path<uint64_t>(
    const int8_t *query, std::vector<std::pair<float, uint64_t>>& result, const size_t K, const uint32_t L);
template DISKANN_DLLEXPORT std::pair<uint32_t, uint32_t> Index<int8_t, uint64_t, uint32_t>::search_with_path<uint32_t>(
    const int8_t *query, std::vector<std::pair<float, uint32_t>>& result, const size_t K, const uint32_t L);
// TagT==uint32_t
template DISKANN_DLLEXPORT std::pair<uint32_t, uint32_t> Index<float, uint32_t, uint32_t>::search_with_path<uint64_t>(
    const float *query, std::vector<std::pair<float, uint64_t>>& result, const size_t K, const uint32_t L);
template DISKANN_DLLEXPORT std::pair<uint32_t, uint32_t> Index<float, uint32_t, uint32_t>::search_with_path<uint32_t>(
    const float *query, std::vector<std::pair<float, uint32_t>>& result, const size_t K, const uint32_t L);
template DISKANN_DLLEXPORT std::pair<uint32_t, uint32_t> Index<uint8_t, uint32_t, uint32_t>::search_with_path<uint64_t>(
    const uint8_t *query, std::vector<std::pair<float, uint64_t>>& result, const size_t K, const uint32_t L);
template DISKANN_DLLEXPORT std::pair<uint32_t, uint32_t> Index<uint8_t, uint32_t, uint32_t>::search_with_path<uint32_t>(
    const uint8_t *query, std::vector<std::pair<float, uint32_t>>& result, const size_t K, const uint32_t L);
template DISKANN_DLLEXPORT std::pair<uint32_t, uint32_t> Index<int8_t, uint32_t, uint32_t>::search_with_path<uint64_t>(
    const int8_t *query, std::vector<std::pair<float, uint64_t>>& result, const size_t K, const uint32_t L);
template DISKANN_DLLEXPORT std::pair<uint32_t, uint32_t> Index<int8_t, uint32_t, uint32_t>::search_with_path<uint32_t>(
    const int8_t *query, std::vector<std::pair<float, uint32_t>>& result, const size_t K, const uint32_t L);


template DISKANN_DLLEXPORT std::pair<uint32_t, uint32_t> Index<float, uint64_t, uint32_t>::search_with_path_early_stop<uint64_t>(
    const float *query, std::vector<std::pair<float, uint64_t>>& result, const size_t K, const uint32_t L, float radius, float scale_factor);
template DISKANN_DLLEXPORT std::pair<uint32_t, uint32_t> Index<float, uint64_t, uint32_t>::search_with_path_early_stop<uint32_t>(
    const float *query, std::vector<std::pair<float, uint32_t>>& result, const size_t K, const uint32_t L, float radius, float scale_factor);
template DISKANN_DLLEXPORT std::pair<uint32_t, uint32_t> Index<uint8_t, uint64_t, uint32_t>::search_with_path_early_stop<uint64_t>(
    const uint8_t *query, std::vector<std::pair<float, uint64_t>>& result, const size_t K, const uint32_t L, float radius, float scale_factor);
template DISKANN_DLLEXPORT std::pair<uint32_t, uint32_t> Index<uint8_t, uint64_t, uint32_t>::search_with_path_early_stop<uint32_t>(
    const uint8_t *query, std::vector<std::pair<float, uint32_t>>& result, const size_t K, const uint32_t L, float radius, float scale_factor);
template DISKANN_DLLEXPORT std::pair<uint32_t, uint32_t> Index<int8_t, uint64_t, uint32_t>::search_with_path_early_stop<uint64_t>(
    const int8_t *query, std::vector<std::pair<float, uint64_t>>& result, const size_t K, const uint32_t L, float radius, float scale_factor);
template DISKANN_DLLEXPORT std::pair<uint32_t, uint32_t> Index<int8_t, uint64_t, uint32_t>::search_with_path_early_stop<uint32_t>(
    const int8_t *query, std::vector<std::pair<float, uint32_t>>& result, const size_t K, const uint32_t L, float radius, float scale_factor);
// TagT==uint32_t
template DISKANN_DLLEXPORT std::pair<uint32_t, uint32_t> Index<float, uint32_t, uint32_t>::search_with_path_early_stop<uint64_t>(
    const float *query, std::vector<std::pair<float, uint64_t>>& result, const size_t K, const uint32_t L, float radius, float scale_factor);
template DISKANN_DLLEXPORT std::pair<uint32_t, uint32_t> Index<float, uint32_t, uint32_t>::search_with_path_early_stop<uint32_t>(
    const float *query, std::vector<std::pair<float, uint32_t>>& result, const size_t K, const uint32_t L, float radius, float scale_factor);
template DISKANN_DLLEXPORT std::pair<uint32_t, uint32_t> Index<uint8_t, uint32_t, uint32_t>::search_with_path_early_stop<uint64_t>(
    const uint8_t *query, std::vector<std::pair<float, uint64_t>>& result, const size_t K, const uint32_t L, float radius, float scale_factor);
template DISKANN_DLLEXPORT std::pair<uint32_t, uint32_t> Index<uint8_t, uint32_t, uint32_t>::search_with_path_early_stop<uint32_t>(
    const uint8_t *query, std::vector<std::pair<float, uint32_t>>& result, const size_t K, const uint32_t L, float radius, float scale_factor);
template DISKANN_DLLEXPORT std::pair<uint32_t, uint32_t> Index<int8_t, uint32_t, uint32_t>::search_with_path_early_stop<uint64_t>(
    const int8_t *query, std::vector<std::pair<float, uint64_t>>& result, const size_t K, const uint32_t L, float radius, float scale_factor);
template DISKANN_DLLEXPORT std::pair<uint32_t, uint32_t> Index<int8_t, uint32_t, uint32_t>::search_with_path_early_stop<uint32_t>(
    const int8_t *query, std::vector<std::pair<float, uint32_t>>& result, const size_t K, const uint32_t L, float radius, float scale_factor);


template DISKANN_DLLEXPORT std::pair<uint32_t, uint32_t> Index<float, uint64_t, uint32_t>::search_with_path_early_stop<uint64_t>(
    const float *query, std::vector<std::pair<float, uint64_t>>& result, const size_t K, const uint32_t L, std::vector<float> &radius, float scale_factor);
template DISKANN_DLLEXPORT std::pair<uint32_t, uint32_t> Index<float, uint64_t, uint32_t>::search_with_path_early_stop<uint32_t>(
    const float *query, std::vector<std::pair<float, uint32_t>>& result, const size_t K, const uint32_t L, std::vector<float> &radius, float scale_factor);
template DISKANN_DLLEXPORT std::pair<uint32_t, uint32_t> Index<uint8_t, uint64_t, uint32_t>::search_with_path_early_stop<uint64_t>(
    const uint8_t *query, std::vector<std::pair<float, uint64_t>>& result, const size_t K, const uint32_t L, std::vector<float> &radius, float scale_factor);
template DISKANN_DLLEXPORT std::pair<uint32_t, uint32_t> Index<uint8_t, uint64_t, uint32_t>::search_with_path_early_stop<uint32_t>(
    const uint8_t *query, std::vector<std::pair<float, uint32_t>>& result, const size_t K, const uint32_t L, std::vector<float> &radius, float scale_factor);
template DISKANN_DLLEXPORT std::pair<uint32_t, uint32_t> Index<int8_t, uint64_t, uint32_t>::search_with_path_early_stop<uint64_t>(
    const int8_t *query, std::vector<std::pair<float, uint64_t>>& result, const size_t K, const uint32_t L, std::vector<float> &radius, float scale_factor);
template DISKANN_DLLEXPORT std::pair<uint32_t, uint32_t> Index<int8_t, uint64_t, uint32_t>::search_with_path_early_stop<uint32_t>(
    const int8_t *query, std::vector<std::pair<float, uint32_t>>& result, const size_t K, const uint32_t L, std::vector<float> &radius, float scale_factor);
// TagT==uint32_t
template DISKANN_DLLEXPORT std::pair<uint32_t, uint32_t> Index<float, uint32_t, uint32_t>::search_with_path_early_stop<uint64_t>(
    const float *query, std::vector<std::pair<float, uint64_t>>& result, const size_t K, const uint32_t L, std::vector<float> &radius, float scale_factor);
template DISKANN_DLLEXPORT std::pair<uint32_t, uint32_t> Index<float, uint32_t, uint32_t>::search_with_path_early_stop<uint32_t>(
...
float early_stop_factor,
int fd,
std::unique_ptr<AsyncIOManager> &io_mgr,
IDType *indices, float *distances = nullptr
 */
template DISKANN_DLLEXPORT std::pair<uint32_t, uint32_t> Index<float, uint64_t, uint32_t>::search_along_path_early_stop_disk<uint64_t>(
    const float *query,
    size_t K,
    size_t L,
    std::vector<uint32_t>& agg2id,
    ...
    float early_stop_factor,
    int fd,
    std::unique_ptr<AsyncIOManager<float>> &io_mgr,
    uint64_t *indices, float *distances);


template DISKANN_DLLEXPORT std::pair<uint32_t, uint32_t> Index<float, uint64_t, uint32_t>::search_along_path_early_stop_disk<uint32_t>(
    const float *query,
    size_t K,
    size_t L,
    std::vector<uint32_t>& agg2id,
...
template DISKANN_DLLEXPORT std::pair<uint32_t, uint32_t> Index<uint8_t, uint64_t, uint32_t>::search_along_path_early_stop_disk<uint64_t>(
    const uint8_t *query,
    size_t K,
    size_t L,
    std::vector<uint32_t>& agg2id,
    ...
    float early_stop_factor,
    int fd,
    std::unique_ptr<AsyncIOManager<uint8_t>> &io_mgr,
    uint64_t *indices, float *distances);

template DISKANN_DLLEXPORT std::pair<uint32_t, uint32_t> Index<uint8_t, uint64_t, uint32_t>::search_along_path_early_stop_disk<uint32_t>(
    const uint8_t *query,
    size_t K,
    size_t L,
    std::vector<uint32_t>& agg2id,
...
template DISKANN_DLLEXPORT std::pair<uint32_t, uint32_t> Index<int8_t, uint64_t, uint32_t>::search_along_path_early_stop_disk<uint64_t>(
    const int8_t *query,
    size_t K,
    size_t L,
    std::vector<uint32_t>& agg2id,
    ...
    float early_stop_factor,
    int fd,
    std::unique_ptr<AsyncIOManager<int8_t>> &io_mgr,
    uint64_t *indices, float *distances);

template DISKANN_DLLEXPORT std::pair<uint32_t, uint32_t> Index<int8_t, uint64_t, uint32_t>::search_along_path_early_stop_disk<uint32_t>(
    const int8_t *query,
    size_t K,
    size_t L,
    std::vector<uint32_t>& agg2id,
    ...
    uint32_t *indices, float *distances);
// TagT==uint32_t
template DISKANN_DLLEXPORT std::pair<uint32_t, uint32_t> Index<float, uint32_t, uint32_t>::search_along_path_early_stop_disk<uint64_t>(
    const float *query,
    size_t K,
    size_t L,
    std::vector<uint32_t>& agg2id,
    ...
    std::unique_ptr<AsyncIOManager<float>> &io_mgr,
    uint64_t *indices, float *distances);
template DISKANN_DLLEXPORT std::pair<uint32_t, uint32_t> Index<float, uint32_t, uint32_t>::search_along_path_early_stop_disk<uint32_t>(
    const float *query,
    size_t K,
    size_t L,
    std::vector<uint32_t>& agg2id,
    ...
    std::unique_ptr<AsyncIOManager<float>> &io_mgr,
    uint32_t *indices, float *distances);
template DISKANN_DLLEXPORT std::pair<uint32_t, uint32_t> Index<uint8_t, uint32_t, uint32_t>::search_along_path_early_stop_disk<uint64_t>(
    const uint8_t *query,
    size_t K,
    size_t L,
    std::vector<uint32_t>& agg2id,
    ...
    std::unique_ptr<AsyncIOManager<uint8_t>> &io_mgr,
    uint64_t *indices, float *distances);
template DISKANN_DLLEXPORT std::pair<uint32_t, uint32_t> Index<uint8_t, uint32_t, uint32_t>::search_along_path_early_stop_disk<uint32_t>(
    const uint8_t *query,
    size_t K,
    size_t L,
    std::vector<uint32_t>& agg2id,
    ...
    std::unique_ptr<AsyncIOManager<uint8_t>> &io_mgr,
    uint32_t *indices, float *distances);
template DISKANN_DLLEXPORT std::pair<uint32_t, uint32_t> Index<int8_t, uint32_t, uint32_t>::search_along_path_early_stop_disk<uint64_t>(
    const int8_t *query,
    size_t K,
    size_t L,
    std::vector<uint32_t>& agg2id,
    ...
    std::unique_ptr<AsyncIOManager<int8_t>> &io_mgr,
    uint64_t *indices, float *distances);
template DISKANN_DLLEXPORT std::pair<uint32_t, uint32_t> Index<int8_t, uint32_t, uint32_t>::search_along_path_early_stop_disk<uint32_t>(
    const int8_t *query,
    size_t K,
    size_t L,
    std::vector<uint32_t>& agg2id,
