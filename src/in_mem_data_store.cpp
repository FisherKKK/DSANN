...
{
    size_t npts, ndim;
    // 从文件中copy对齐数据, 这里保证所有的数据都是_aligned_dim对齐, offset就是读取的为止
    // 这里完成将数据读入到_data中
    ...
    // 对数据进行预处理
    if (_distance_fn->preprocessing_required()) //TODO: PAG CANCEL
    {
...
// 这里本质上就是执行了内存copy, 将数据从data => dest
...
template <typename data_t> location_t InMemDataStore<data_t>::calculate_medoid_num(size_t num) const {
    ...
    for (size_t i = 0; i < num; i++)
        for (size_t j = 0; j < _aligned_dim; j++)
            // 所有数据的第j位
            // 第i个数据为_data[i * _aligned_dim]
            center[j] += (float)_data[i * _aligned_dim + j];

    // 均值计算质心, capacity相当于总共的数据量
    for (size_t j = 0; j < _aligned_dim; j++)
        center[j] /= (float)num;
