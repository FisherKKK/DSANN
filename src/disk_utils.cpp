 ...
 *      2. 并发搜索构建索引
 */
DISKANN_DLLEXPORT int merge_index_distributed(const std::vector<std::unique_ptr<diskann::Index<float>>> &indices,
                                              const std::string &index_path_prefix, const std::string &idmaps_prefix,
                                              const std::string &idmaps_suffix, const uint64_t nshards, uint32_t max_degree,
                                              const std::string &output_vamana) {

    int L = 30;
    int K = max_degree / nshards;
    std::vector<std::vector<uint32_t>> idmaps(nshards);
    diskann::cout << "Loading id map....\n";
    for (uint64_t shard = 0; shard < nshards; shard++)
    {
        // 读取该索引对应的id
        // 每一个shard中sub -> real id
        read_idmap(idmaps_prefix + std::to_string(shard) + idmaps_suffix, idmaps[shard]);
    }

    // find max node id
    // 最大的节点id, 本质上就是最大的节点数目
    size_t nnodes = 0;
    size_t nelems = 0;
    ...
    std::vector<std::vector<uint32_t>> graph(nelems);
    for (size_t i = 0; i < nelems; i++) {
        graph[i].resize(K * nshards);
    }

    // 对每一个分区进行并行
#pragma omp parallel for
    for (size_t shard = 0; shard < nshards; shard++) {
        diskann::cout << "Dealing with shard: #" << shard << "\n";
        std::string shard_base_file = index_path_prefix + "_subshard-" + std::to_string(shard) + ".bin";
        float *query = nullptr;
        size_t query_num, query_dim, query_aligned_dim, gt_num, gt_dim;
        diskann::load_aligned_bin<float>(shard_base_file, query, query_num, query_dim, query_aligned_dim);


        std::vector<uint32_t> query_result(query_num * K * nshards);
...
#pragma omp parallel for
            for (size_t si = 0; si < nshards; si++) {
                uint32_t *idx = query_result.data() + i * nshards * K + si * K;

                indices[si]->search(query + i * query_aligned_dim, K, L, idx);

                // 接下来我们还需要将id -> 真实id
                for (size_t j = 0; j < K; j++)
                    idx[j] = idmaps[si][idx[j]];
            }

            size_t real_id = idmaps[shard][i];
            memcpy(graph[real_id].data(), query_result.data() + i * nshards * K, nshards * sizeof(uint32_t) * K);
        }
    }

    diskann::cout << "Searching end for all shards\n";


    // 获取vamana_metadata_size
    ...
    uint32_t output_width = K * nshards;
    merged_vamana_writer.write((char *)&output_width, sizeof(uint32_t));

    uint32_t medoid = indices[nshards - 1]->get_start();
    medoid = idmaps[nshards - 1][medoid];

    merged_vamana_writer.write((char *)&medoid, sizeof(uint32_t));
    merged_vamana_writer.write((char *)&merged_index_frozen, sizeof(size_t));

    for (size_t i = 0; i < nelems; i++) {
        uint32_t  nnbrs = K * nshards;
        merged_vamana_writer.write((char *)&nnbrs, sizeof(uint32_t));
        merged_vamana_writer.write((char *)graph[i].data(), nnbrs * sizeof(uint32_t));
...
/** 合并分区索引, 主要流程如下:
 *      1. 读取每一个索引构建shard => [], id => shard映射
 *      2. 寻找每个真实id下的neighbor, 然后直接将真实的neighbor进行拼接
 *      3. 最后将拼接完成的neighbor放回到文件中
 */
int merge_index(const std::string &vamana_prefix, const std::string &vamana_suffix, const std::string &idmaps_prefix,
                const std::string &idmaps_suffix, const uint64_t nshards, uint32_t max_degree,
                const std::string &output_vamana, const std::string &medoids_file)
{
    ...
    // 按照node_id进行排序, 这样的话就可以根据node_id来获取对应的shard
    ...
    // 读取每一个shard vamana, width实际上就是度数?
    ...
    merged_vamana_writer.write((char *)&output_width, sizeof(uint32_t));
    //! 缺少写起始点id的过程, 但是后面用的是最后一个memoid: out.write((char *)&start, sizeof(uint32_t));
    // 写质心文件
    ...
    // 对每个shard进行操作, memoid是每个index的入口点
    for (uint64_t shard = 0; shard < nshards; shard++)
    {
        uint32_t medoid;
        // read medoid
    ...
    for (const auto &id_shard : node_shard)
    {
        ...
        // rename nodes
        //! 除非说构造index的时候, 对应的id并非真实的id
        // 也就是说每个sub_index中实际上是sub_id
        // sub_id => real_id 采用neighbor_id => idmaps[shard_id][shard_nhood[neighbor_id]获取真实id
...
/** 这个函数暂时看起来是构建vamana index, 但是具体还要细看:
 *      1. 如果index可以容纳在内存中, 直接在采用build_memory_index的一套东西
 *      2. 否则, 采用k-means的方法将数据进行聚类, 然后分片存储数据
 *      3. 对分片的数据建立vamna索引
 *      4. 对分片的索引进行合并
 */
                              ...
                              const uint32_t Lf)
{
    ...
    //! 这一步相当于index无法直接填入内存中, 那么就先对数据进行分片, 采用k-means
    // 每个数据最多会被存储到k个cluster对应的文件中
    ...
    // 这一步相当于将index ram数据进行划分
    // 本质上下面的逻辑就是将数据进行划分, 然后每个分区中构建索引

    // 首先将数据进行划分, 并保存在文件中, 一共划分了num_parts个部分
    int num_parts =
...
/** 创建磁盘布局:
 *      Sector:
 *          Node1: origin vector | neighbor size | neighbor_id | neighbor_id | ... |
 *          Node2: ...
 *          Node3: ...
 */
template <typename T>
                        ...
                        const std::string reorder_data_file)
{
    ...
    //! Layout: vector neighbor_number id * number
        ...
        for (uint64_t sector = 0; sector < n_sectors; sector++)
        {
                 ...
                 sector_node_id++)
            {
                ...
                *(uint32_t *)(node_buf.get() + ndims_64 * sizeof(T)) = (std::min)(nnbrs, width_u32);
                ...
                // 将node_buf中的内容拷贝到sector_node_buf中
                memcpy(sector_node_buf, node_buf.get(), max_node_len);
                cur_node_id++;
            }
            // 最后将sector_buf中的内容写入磁盘
            // flush sector to disk
            diskann_writer.write(sector_buf.get(), defaults::SECTOR_LEN);
        }
    }
    else
    { // Write multi-sector nodes, 这里就是说每个node需要多个sector
                     ...
                     const uint32_t Lf)
{
    ...
    if (param_list.size() < 5 || param_list.size() > 9)
    {
        diskann::cout << "Correct usage of parameters is R (max degree)\n"
                         ...
                         ": optional parameter, use only when using disk PQ\n"
                         "build_PQ_byte (number of PQ bytes for index build; set 0 to use "
    ...
    // 获取索引内存预算, 以B的形式返回结果, 表示最后内存中的索引大小
    ...
    // 内存中的PQ块数, 表示什么呢 => 有可能是PQ的维度, 但是采用字节表示, 每个向量的最大大小
    // 这里表示的实际上就是每个向量所允许的最大字节数
...
// This code should only be available in the "build" DLL.
    ...
    if (!use_disk_pq)
    {
        // 创建磁盘布局, 本质上就是按照sector来保存批量的node
