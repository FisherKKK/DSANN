...
int  main(int argc, char *argv[]) {
    ...
    /** 事先将index放到vector中 **/
    std::vector<std::unique_ptr<diskann::Index<float>>> indices(num_centers);
...
#pragma omp parallel for schedule(static, 1)
    for (size_t part = 0; part < num_centers; part++)
    {
        ...
        auto _index = std::make_unique<diskann::Index<float>>(metric, shard_base_dim, shard_base_pts,
                                                              ...
                                                              0, false);
        indices[part] = std::move(_index);
        indices[part]->build(shard_base_file.c_str(), shard_base_pts);

//        indices[part]->save(shard_index_file.c_str());
...
//        std::remove(shard_base_file.c_str());
    ...
    /** 索引合并 **/
    diskann::merge_index_distributed(indices, index_path_prefix, index_path_prefix + "_subshard-",
                                     "_ids_uint32.bin", num_centers, R, index_path_prefix);
