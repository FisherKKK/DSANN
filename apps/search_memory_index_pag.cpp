                        ...
                        const std::vector<std::string> &query_filters, const float fail_if_recall_below)
{
    using TagT = uint32_t;
    // Load the query file
    dist_t *query = nullptr;
    uint32_t *gt_ids = nullptr;
    float *gt_dists = nullptr;
    size_t query_num, query_dim, query_aligned_dim, gt_num, gt_dim;
    diskann::load_aligned_bin<dist_t>(query_file, query, query_num, query_dim, query_aligned_dim);
//    query_num = 1000;
    if (std::is_same_v<dist_t, float> && metric == diskann::Metric::COSINE) {
    ...
    auto index = std::make_unique<diskann::PointAggregationGraph<dist_t>>();
...
//                                   .second;
                ...
                if (index->RoutingPathSearch()) {
                    auto retval = index->SearchInMemoryByRoute(query + i * query_aligned_dim, recall_at, L,
                                                 query_result_ids[test_id].data() + i * recall_at);
                    cmp_stats[i] = retval.second;
                } else {
                    size_t nprobe = L;
//                    auto rL = std::min(2 * L, (*std::max(Lvec.begin(), Lvec.end())));
                    auto retval = index->SearchInMemoryByTopK(query + i * query_aligned_dim, recall_at, L, nprobe,
                                                query_result_ids[test_id].data() + i * recall_at);
                    cmp_stats[i] = retval.second;
