...
{
  public:
    // capacity: 总的点的数目, 这个初始化函数为真正的数据创建内存空间
    // 但是当前数据集中没有任何数据
    ...
    virtual void get_vector(const location_t i, data_t **target) override {
        *target = _data + i * _aligned_dim;
    }
    ...
    virtual location_t calculate_medoid_num(size_t num) const override;
