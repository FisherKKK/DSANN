...
template <typename T, typename TagT = uint32_t, typename LabelT = uint32_t> class Index : public AbstractIndex
{
  ...
  public:
    ...
    // 函数构造: config, 原始数据集(共享指针), 图数据集(唯一指针), PQ数据集(共享指针), 目前为止所有东西都是空的
    ...
    DISKANN_DLLEXPORT void add_neighbours_random(location_t id, std::vector<uint32_t> &rec) {
        auto neighbors = _graph_store->get_neighbours(id);
        std::vector<uint32_t> target;
        for (auto nid: rec) {
            if (std::find(neighbors.begin(), neighbors.end(), nid) == neighbors.end())
                target.emplace_back(nid);
        }
    }

    DISKANN_DLLEXPORT void prune_neighbors(location_t id, std::vector<uint32_t> &prune_list, InMemQueryScratch<T> *scratch) {
        std::vector<Neighbor> pool(prune_list.size());
        for (auto nid: prune_list) {
            pool.emplace_back(nid, _data_store->get_distance(nid, id));
        }

        std::sort(pool.begin(), pool.end());
        prune_list.clear();
        occlude_list(id, pool, _indexingAlpha, _indexingRange, _indexingMaxC, prune_list, scratch);
    }

    DISKANN_DLLEXPORT void prune_neighbors_for(location_t id, std::vector<uint32_t> &red_list) {
//        std::shared_lock<std::shared_timed_mutex> shared_ul(_update_lock);
//        auto neighbors = _graph_store->get_neighbours(id);
//        ScratchStoreManager<InMemQueryScratch<T>> manager(_query_scratch);
//        auto scratch = manager.scratch_space();
//        for (auto nid: red_list) {
//            if (std::find(neighbors.begin(), neighbors.end(), nid) == neighbors.end())
//                neighbors.emplace_back( nid);
//        }
//
//        prune_neighbors(id, neighbors, scratch);
//
//        {
//            LockGuard guard(_locks[id]);
//            _graph_store->clear_neighbours(id);
//            _graph_store->set_neighbours(id, neighbors);
//        }
//        inter_insert(id, neighbors, scratch);
        auto neighbors = _graph_store->get_neighbours(id);
        for (auto rid: red_list) neighbors.push_back(rid);
        std::shuffle(neighbors.begin(), neighbors.end(), std::default_random_engine(time(NULL)));
        while (neighbors.size() > _indexingRange) {
            neighbors.pop_back();
        }
    ...
    DISKANN_DLLEXPORT std::vector<uint32_t> get_neighbors(size_t i) {
        return _graph_store-> get_neighbours(i);
    }

    DISKANN_DLLEXPORT uint32_t get_start() { return _start; }

    DISKANN_DLLEXPORT void update_start() {
        _start = calculate_entry_point();
    }


    DISKANN_DLLEXPORT void get_vector(location_t i, T **dest) {
    ...
    DISKANN_DLLEXPORT std::pair<uint32_t, uint32_t> search_with_path(const T *query, std::vector<std::pair<float, IDType>> &result, const size_t K, const uint32_t L);

    template <typename IDType>
    DISKANN_DLLEXPORT std::pair<uint32_t, uint32_t> search_with_path_early_stop(const T *query,
    ...
    template <typename IDType>
    DISKANN_DLLEXPORT std::pair<uint32_t, uint32_t> search_along_path_early_stop_disk(const T *query,
                                                                                      size_t K,
                                                                                      size_t L,
    ...
    DISKANN_DLLEXPORT void sample_neighbor_distance(location_t id, std::vector<float>& dists);
  ...
  protected:
    ...
    uint32_t calculate_entry_point(size_t num);
    ...
    // iterate with early stop condition
    std::vector<std::pair<float, uint32_t>> path_to_fixed_point_early_stop(InMemQueryScratch<T> *scratch, const uint32_t Lindex,
                                                                           const std::vector<uint32_t> &init_ids, bool use_filter,
                                                                           const std::vector<LabelT> &filters, bool search_invocation,
    ...
    // iterate while loading the disk vector
    std::vector<std::pair<float, uint32_t>> path_to_fixed_point_early_stop_disk(int k,
                                                                                InMemQueryScratch<T> *scratch, const uint32_t Lindex,
                                                                                const std::vector<uint32_t> &init_ids, bool use_filter,
                                                                                const std::vector<LabelT> &filters, bool search_invocation,
                                                                                std::vector<uint32_t >& agg2id,
                                                                                std::vector<std::vector<uint32_t>> &ivf,
    ...
    // Return the path to a query
    std::vector<std::pair<float, uint32_t>> path_to_fixed_point(InMemQueryScratch<T> *scratch, const uint32_t Lindex,
                                                                const std::vector<uint32_t> &init_ids, bool use_filter,
                                                                const std::vector<LabelT> &filters, bool search_invocation);
